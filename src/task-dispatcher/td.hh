...
template <class F>
[[deprecated]] Sync submit(F&& fun) = delete;

template <class F, class FObj, class... Args, cc::enable_if<std::is_member_function_pointer_v<F>> = true>
[[deprecated]] Sync submit(F func, FObj& inst, Args&&... args) = delete;

template <class F, class... Args, cc::enable_if<std::is_invocable_v<F, Args...> && !std::is_member_function_pointer_v<F>> = true>
[[deprecated]] Sync submit(F&& fun, Args&&... args) = delete;

template <class F>
[[deprecated]] Sync submit_n(F&& func, unsigned n) = delete;

template <class T, class F>
[[deprecated]] Sync submit_each_ref(F&& func, cc::span<T> vals) = delete;

template <class T, class F>
[[deprecated]] Sync submit_each_copy(F&& func, cc::span<T> vals) = delete;

template <class F>
[[deprecated]] Sync submit_batched(F&& func, unsigned n, unsigned num_batches_max = td::getNumLogicalCPUCores() * 4) = delete;

template <class F>
[[deprecated]] Sync submit_batched_n(F&& func, unsigned n, unsigned num_batches_max = td::getNumLogicalCPUCores() * 4) = delete;
}
